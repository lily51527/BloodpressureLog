...
class BuildChartDataUseCaseTest {
    ...
    /** xLabels 數量應等於 startMs ~ endMs 之間的天數（含頭尾兩端）。 */
    @Test
    fun `xLabels count matches day range inclusive`() {
        val today = LocalDate.now()

        val (_, labels7) = useCase(emptyList(), epochMillis(today.minusDays(6)), epochMillis(today), TrendMetric.SYSTOLIC)
        val (_, labels14) = useCase(emptyList(), epochMillis(today.minusDays(13)), epochMillis(today), TrendMetric.SYSTOLIC)
        val (_, labels30) = useCase(emptyList(), epochMillis(today.minusDays(29)), epochMillis(today), TrendMetric.SYSTOLIC)
    ...
    fun `empty records produce empty points`() {
        ...
        val (points, _) = useCase(emptyList(), epochMillis(today.minusDays(6)), epochMillis(today), TrendMetric.SYSTOLIC)
    ...
    fun `records outside range are excluded`() {
        ...
        val (points, _) = useCase(records, epochMillis(today.minusDays(6)), epochMillis(today), TrendMetric.SYSTOLIC)
    ...
    fun `metric DIASTOLIC returns correct average`() {
        ...
        val (points, _) = useCase(records, epochMillis(today.minusDays(6)), epochMillis(today), TrendMetric.DIASTOLIC)
    ...
    fun `metric PULSE returns correct average`() {
        ...
        val (points, _) = useCase(records, epochMillis(today.minusDays(6)), epochMillis(today), TrendMetric.PULSE)
    ...
    fun `dayIndex is relative to startDate`() {
        ...
        assertThat(sortedPoints[1].first).isEqualTo(6f)   // today → dayIndex 6
    }
    ...
    fun `records within 14-day range are included`() {
        ...
        val (points, _) = useCase(records, epochMillis(today.minusDays(13)), epochMillis(today), TrendMetric.SYSTOLIC)
     ...
     * 以 7 天區間為例：startDate = today.minusDays(6)，minusDays(7) 超出範圍。
     */
    @Test
    fun `record on startDate is included and record before startDate is excluded`() {
        ...
        val (points, _) = useCase(records, epochMillis(startDate), epochMillis(today), TrendMetric.SYSTOLIC)
    ...
    /**
     * endDate 早於 startDate 時（理論上 UI 已透過 SelectableDates 擋掉），
     * UseCase 應回傳空的圖表資料點與 xLabels，不應拋出例外。
     */
    @Test
    fun `endDate before startDate returns empty result`() {
        val today = LocalDate.now()
        val records = listOf(record(120, 80, 70, today))

        val (points, labels) = useCase(records, epochMillis(today), epochMillis(today.minusDays(1)), TrendMetric.SYSTOLIC)
    ...
    fun `xLabels have correct format and are not blank`() {
        ...
        val (_, labels) = useCase(emptyList(), epochMillis(startDate), epochMillis(today), TrendMetric.SYSTOLIC)
